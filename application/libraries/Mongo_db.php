 ...
 *
 * @package CodeIgniter
 * @author Erwin Houtsma
 * @copyright Copyright (c) 2015, Erwin Houtsma.
    ...
    private function prepare() {
        ...
        if (isset($this->config[$this->activate]) == TRUE) {
            if (empty($this->config[$this->activate]['hostname'])) {
                ...
                if (is_array($this->config[$this->activate]['hostname'])) {
                    $this->hostname = [];
                    foreach ($this->config[$this->activate]['hostname'] as &$hostname)
    ...
    private function connect() {
        ...
        try {
            ...
            if (is_array($this->hostname)) {
                $comma = '';
                foreach ($this->hostname as $hostname) {
                    $dsn .= $comma . "{$hostname}:{$this->port}";
                    $comma = ',';
                }
            } else
                $dsn .= "{$this->hostname}:{$this->port}";
            $dsn .= "/{$this->database}";
            ...
            if (!empty($this->config[$this->activate]['replicaSet'])) {
                $options['replicaSet'] = $this->config[$this->activate]['replicaSet'];
            }
...
//$this->db->selectCollection($collection);
    ...
    public function distinct($collection = "", $field = "") {
        ...
        try {
            $documents = $this->db->selectCollection($collection)->distinct($field, $this->wheres);
            $this->_clear();
            if ($this->return_as == 'object') {
    ...
    public function update($collection = "", $options = array()) {
        ...
        try {
            ...
            $this->db->selectCollection($collection)->updateOne($this->wheres, $this->updates, $options);
            $this->_clear();
            return (TRUE);
        } catch (MongoDB\Driver\Exception\BulkWriteException $e) {
            if (isset($this->debug) == TRUE && $this->debug == TRUE) {
    ...
    public function update_all($collection = "", $data = array(), $options = array()) {
        ...
        try {
            ...
            $this->db->selectCollection($collection)->updateMany($this->wheres, $this->updates, $options);
            $this->_clear();
            return (TRUE);
        } catch (MongoDB\Driver\Exception\BulkWriteException $e) {
            if (isset($this->debug) == TRUE && $this->debug == TRUE) {
    ...
    public function delete($collection = "") {
        ...
        try {
            $this->db->selectCollection($collection)->deleteOne($this->wheres, array('w' => $this->write_concerns, 'j' => $this->journal, 'justOne' => TRUE));
            $this->_clear();
            return (TRUE);
        } catch (MongoDB\Driver\Exception\BulkWriteException $e) {
            if (isset($this->debug) == TRUE && $this->debug == TRUE) {
    ...
    public function delete_all($collection = "") {
        ...
        try {
            $this->db->selectCollection($collection)->deleteMany($this->wheres, array('w' => $this->write_concerns, 'j' => $this->journal, 'justOne' => FALSE));
            $this->_clear();
            return (TRUE);
        } catch (MongoDB\Driver\Exception\BulkWriteException $e) {
            if (isset($this->debug) == TRUE && $this->debug == TRUE) {
    ...
    public function aggregate($collection, $operation) {
        ...
        try {
            $documents = $this->db->selectCollection($collection)->aggregate($operation);
            $this->_clear();
            if ($this->return_as == 'object') {
                return (object) $documents;
            } else {
    ...
    public function add_index($collection = "", $keys = array(), $options = array()) {
        ...
        try {
            $this->db->selectCollection($collection)->createIndex($keys, $options);
            $this->_clear();
            return ($this);
    ...
    private function explain($cursor, $collection, $aggregate = null) {
        array_push($this->benchmark, array(
